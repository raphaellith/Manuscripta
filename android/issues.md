...
**Requirements:**
- Define all entity classes for local persistence (Room)
...
**Tasks:**
- Create `MaterialEntity.java` with fields: id, type (enum: QUIZ, WORKSHEET, POLL), title, content, metadata, vocabularyTerms (JSON array), timestamp
...
**Tasks:**
- Create `QuestionEntity.java` with fields: id, materialId (foreign key), questionText, questionType, options (JSON), correctAnswer
...
**Tasks:**
- Create `ResponseEntity.java` with fields: id, questionId (foreign key), selectedAnswer, isCorrect, timestamp, synced
...
**Tasks:**
- Create `SessionEntity.java` with fields: id, materialId, startTime, endTime, status, deviceId
...
**Tasks:**
- Create `MaterialDto.java` with @SerializedName annotations
...
- Create mapper methods (DTO → Domain)
- Write unit tests
...
**Tasks:**
- Create `ResponseDto.java` (request)
