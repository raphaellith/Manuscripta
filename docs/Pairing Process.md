...
(2) The Android device, when instructed by the user to pair with the Windows device, on discovery of a message as specified in (1), must:
    ...
    (c) Make a POST request to the `/pair` endpoint, as specified in `API Contract.md` §2.4, with a body containing the deviceId and device name.
