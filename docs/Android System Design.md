...
erDiagram
    ...
    FEEDBACK {
        ...
        string text "optional textual feedback"
        int marks "optional numerical marks"
    }
...
### 2.6 Feedback Retrieval (Heartbeat-Triggered)
...
sequenceDiagram
    ...
    Note over A,W: Teacher marks response on Windows
    
    loop Heartbeat
        A->>W: STATUS_UPDATE (0x10)
        
        alt Feedback available
            W->>TCP: RETURN_FEEDBACK (0x07)
            TCP->>FR: onFetchFeedbackSignal()
            FR->>HTTP: GET /feedback/{deviceId}
            HTTP->>W: HTTP Request
            W->>HTTP: 200 OK [FeedbackEntity array]
            HTTP->>FR: Return feedback list
            FR->>FR: Save to Room DB
            FR->>TCP: Send FEEDBACK_ACK (0x13)
            TCP->>W: FEEDBACK_ACK
            FR->>A: Notify UI of new feedback
        end
    end
```
...
classDiagram
    ...
    class FeedbackRepository {
        <<interface>>
        +getFeedbackForResponse(responseId) Feedback?
        +getAllFeedback() Flow~List~Feedback~~
        +onFetchFeedbackSignal()
    }
    ...
    class ApiService {
        ...
        +getFeedback(deviceId) Call~List~FeedbackDto~~
    }
...
graph LR
    ...
    subgraph "Server → Client (TCP)"
        ...
        RETFEED[0x07 RETURN_FEEDBACK<br/>No operand]
    end
    ...
    subgraph "Client → Server (TCP)"
        ...
        FEEDACK[0x13 FEEDBACK_ACK<br/>Device ID UTF-8]
    end
...
| Feedback | Windows Server | Teacher-generated feedback |
