...
You must read the diff and understand what the PR does before approving it. Do not delegate your review to an AI tool — asking Copilot or any other assistant whether the code "looks fine" and then merging on that basis is not a review.
...
GitHub issues, PR comments, and review comments are among our primary communication channels. It is essential that every team member is genuinely reading and engaging with what others write.

### Human-written content only

All communication on GitHub — issue comments, PR descriptions, review comments, and discussion replies — must be written by you, in your own words. Do not let an AI assistant draft or post responses on your behalf.

Using AI to help you think through a problem or understand a concept is fine. But when you reply to a teammate, the words need to be yours. If a comment reads like it was generated by a bot, it will be treated as such — that is, as if no response was given.

### Why this matters

Our GitHub threads are how we stay aligned on design decisions, surface problems, and coordinate work. If someone posts an AI-generated reply without reading the original message, the conversation breaks down and decisions get made without genuine consensus.
