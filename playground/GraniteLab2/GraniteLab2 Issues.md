...
Here is the revised, comprehensive issue that covers both the **Matrix overhaul** and the **missing System Prompt controls** across the entire application.

### Issue #9: Universal System Prompt Integration & Matrix Overhaul
**Label:** `Feature` `Core` `Enhancement`

**Description:**
The application currently lacks a critical testing vector: **System Prompts (Personas)**. While the backend supports it, the UI (Chat, Arena, RAG, Vision) does not allow users to define or load a System Prompt (e.g., "You are a strict Python coding assistant").

Additionally, "The Matrix" (Batch Engine) is too rigid. It restricts testing to only pre-saved user prompts and completely ignores System Prompts. We need to make System Prompts a "First Class Citizen" available at *every* point of interaction.

**Tasks:**
- [ ] **UI Component (`modules/prompt_ui.py`):**
    - [ ] Create a reusable `render_prompt_selector()` component.
    - [ ] Features: Tabbed interface for "Saved Personas" (from DB) vs "Custom Input" (Text Area).
    - [ ] Ensure selected prompt state persists in `st.session_state`.
- [ ] **Page Integration (Apply Selector):**
    - [ ] **Chat Lab:** Add selector to sidebar. Pass as `system` role in message history.
    - [ ] **The Arena:** Add "Shared System Prompt" selector to sidebar. Send to *both* models.
    - [ ] **RAG Lab:** Allow overriding the default "You are a helpful assistant..." prompt.
    - [ ] **Vision Lab:** Allow giving the vision model a persona (e.g., "You are an OCR machine").
- [ ] **Matrix Overhaul:**
    - [ ] **Add System Prompt:** Allow selecting a Persona that applies to the entire batch run.
    - [ ] **Flexible User Prompts:**
        - [ ] Keep "Library Select" (Multi-select).
        - [ ] Add "Manual Input" (Text area for ad-hoc prompts, separated by newlines).
    - [ ] **Execution Logic:** Update loop to include the System Message in the API payload.
- [ ] **Persistence:**
    - [ ] Ensure `db.create_experiment` saves the System Prompt used in the `config` JSON so results are reproducible.

**Acceptance Criteria:**
- I can load the "Strict JSON" persona in **Chat Lab** and Granite outputs only JSON.
- I can run a **Matrix Experiment** using the "Creative Writer" persona and 3 manual ad-hoc prompts.
- I can see which System Prompt was used when viewing results in **Analytics**.