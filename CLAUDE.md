# Manuscripta Android Client - Claude Code Instructions
...
di/               → Hilt dependency injection modules
  ...
  └── ServiceModule    → PairingManager, KioskManager, TextToSpeechManager
network/          → Retrofit API interfaces, DTOs (*Dto.java for JSON serialisation)
...
- Do not use git write commands such as commit, push, etc. - only commands that read from git unless specified otherwise.
- When asked to update git issues, read existing 'android' labelled issues for writing/style reference and always use UK English.
