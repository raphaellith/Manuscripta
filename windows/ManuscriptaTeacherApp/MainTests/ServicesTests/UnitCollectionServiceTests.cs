...
public class UnitCollectionServiceTests
{
    ...
    [Fact]
    public async Task GetByIdAsync_ExistingId_ReturnsUnitCollection()
    {
        // Arrange
        var id = Guid.NewGuid();
        var unitCollection = new UnitCollectionEntity(id, "Test");
        _mockUnitCollectionRepo.Setup(r => r.GetByIdAsync(id))
    ...
    [Fact]
    public async Task GetAllAsync_ReturnsAllUnitCollections()
    {
        // Arrange
        var collections = new List<UnitCollectionEntity>
        {
            new UnitCollectionEntity(Guid.NewGuid(), "Collection 1"),
            new UnitCollectionEntity(Guid.NewGuid(), "Collection 2")
        };
        _mockUnitCollectionRepo.Setup(r => r.GetAllAsync())
    ...
    public async Task GetAllAsync_EmptyRepository_ReturnsEmptyCollection()
    {
        // Arrange
        _mockUnitCollectionRepo.Setup(r => r.GetAllAsync())
            .ReturnsAsync(new List<UnitCollectionEntity>());
