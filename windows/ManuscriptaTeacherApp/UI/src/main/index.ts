...
// Token map for secure file access: maps tokens to file paths selected via show-open-dialog.
// Tokens expire after 5 minutes to prevent stale references.
const FILE_TOKEN_EXPIRY_MS = 5 * 60 * 1000;
const MAX_FILE_READ_SIZE_BYTES = 50 * 1024 * 1024; // 50 MB
const fileTokenMap = new Map<string, { filePath: string; expires: number }>();

function cleanupExpiredTokens(): void {
  const now = Date.now();
  for (const [token, entry] of fileTokenMap) {
    ...
    callback({
      responseHeaders: {
        ...
        'Content-Security-Policy': [
          ...
          `connect-src 'self' ${backendUrl} ${backendWsUrl} https://fonts.googleapis.com https://fonts.gstatic.com https://unpkg.com https: http: blob:;`
        ]
      }
    });
  });
...
// Get backend port for SignalR connection
// Per FrontendWorkflowSpecifications §2ZA(8)(c)(iv): Expose active port to renderer
ipcMain.handle('get-backend-port', () => {
  return backendProcessManager.getActivePort();
});
...
// Show open dialog for file picker — returns file tokens for secure read-file-buffer access
ipcMain.handle('show-open-dialog', async (_event, options: Electron.OpenDialogOptions) => {
  const result = await dialog.showOpenDialog(options);
  cleanupExpiredTokens();
  const fileTokens: string[] = [];
  if (!result.canceled) {
    const expires = Date.now() + FILE_TOKEN_EXPIRY_MS;
    for (const filePath of result.filePaths) {
      const token = crypto.randomUUID();
      fileTokenMap.set(token, { filePath, expires });
      fileTokens.push(token);
    }
  }
  return { canceled: result.canceled, filePaths: result.filePaths, fileTokens };
});
...
// Read file as buffer using a secure token from show-open-dialog (per §4AA(2)(b)).
// Only files selected by the user via the native dialog can be read.
// Uses async I/O to avoid blocking the main process event loop.
ipcMain.handle('read-file-buffer', async (_event, fileToken: string) => {
  cleanupExpiredTokens();
  const entry = fileTokenMap.get(fileToken);
  if (!entry) {
    throw new Error('Invalid or expired file token. Please select the file again.');
  }
  // Consume the token — each token can only be used once
  fileTokenMap.delete(fileToken);

  const filePath = entry.filePath;
  const stat = await fs.stat(filePath);
  if (stat.size > MAX_FILE_READ_SIZE_BYTES) {
    throw new Error(`File is too large (${Math.round(stat.size / (1024 * 1024))}MB). Maximum allowed size is ${MAX_FILE_READ_SIZE_BYTES / (1024 * 1024)}MB.`);
  }
