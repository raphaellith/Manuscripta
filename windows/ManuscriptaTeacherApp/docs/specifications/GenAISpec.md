...
(3) All calls to Granite must be performed by calling a method belonging to one of the following classes. All classes listed below must inherit the superclass `AiWorkflowService`.
...
(1) `AiWorkflowService` must provide the following methods.
    ...
    (a) `public async Task<string> GetChatResponseAsync(string prompt)`: After receiving a prompt,
        ...
        (i) runs `ollama list` to verify that Granite 4.0 (named `granite4`) is locally available via Ollama, and runs `ollama pull granite4` to install the model if it is not.
        ...
        (iii) asynchronously returns the chat response generated by Granite.
