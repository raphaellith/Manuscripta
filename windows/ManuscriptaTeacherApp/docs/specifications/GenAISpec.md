...
(1) `AiWorkflowService` must provide the following methods.
    ...
    (a) `public async Task<string> GetChatResponseAsync(string prompt)`: After receiving a prompt,
        ...
        (i) verifies that Granite 4.0 (named `granite4`) is locally available via Ollama by running `ollama list`, and runs `ollama pull granite4` to install the model if it is not; and

        (ii) asynchronously returns the output generated by Granite.
