...
(2) When creating a material, the front-end should -
    ...
    (b) Prompt the user to select the method of creation, through one of the following initial means:
        ...
        (i) AI generation based on description and templates. See Section 4B.

        (ii) Manual creation. This bypasses the AI creation process specified in Section 4B.
    
    (c) The user should be informed that they will still be able to make manual edits, or invoke the AI assistant, after the initial edit.
...
(1) When the user selects AI generation, the frontend should collect the following information:
    ...
    (c) The type of the material:

        (i) (Question: Does it make sense for polls to be generated by AI? Should this selection be made before selection of the method of creation?) Poll.

        (ii) Reading.

        (iii) Worksheet.
    
    (d) Where the material is a worksheet, whether it is intended to be a quiz with only multiple choice questions.

    (e) The reading age and the actual age group of the intended audience.

    (f) (Question: Should we seek to prompt the AI on the desired length of a material?)

(2) Once information in subsection (1) have been collected, the AI module shall be called [Insert Reference to Handler of the Main Component Once Finished] to generate a draft of the material. The draft shall be displayed to the user in the editor modal, as specified in Section 4C. The reading age and the actual age metadata shall be persisted by calling the update material method specified in S1(1)(d) of this document.



## Section 4C - Editor Modal

(1) The front end shall provide a uniform editor modal for editing all types of materials on a what-you-see-is-what-you-get basis. It shall as a minimum, support rendering and editing of all language features specified in the Material Encoding Specification. It shall also suppport rendering, insertion, relocation and deletion of embedded questions, as well as pictures and PDFs.

(2) The editor modal shall provide a means through which the user can invoke the AI assistant to make changes to the material.

(3) The editor modal shall provide a means through which the user can modify the reading age and actual age metadata of the material.

(4) The editor modal shall auto-save its contents at regular intervals, using the appropriate update methods.
