...
(1) The frontend JavaScript client must include the following handlers.
    ...
    (h) Handlers for generation streaming.
        ...
        (i) `OnGenerationProgress`, with parameters `token` (string), `isThinking` (bool), `done` (bool), and `isQueryingSourceDocuments` (bool): Notifies the frontend that a generation chunk has been received from the AI model. The `token` parameter contains the text fragment. The `isThinking` parameter indicates whether the token is part of the model's chain-of-thought reasoning. The `done` parameter indicates whether the stream has completed. The `isQueryingSourceDocuments` parameter indicates whether retrieval from source documents is currently in progress. During retrieval-augmented generation, the backend shall set `isQueryingSourceDocuments = true` whilst querying source documents, and shall set `isQueryingSourceDocuments = false` once source-document querying has completed and before continuing generation progress updates. User-facing wording for this state shall be determined by the frontend. See GenAISpec §3H(5)(a).
